...
const CompanionComponent = ({ companionId, userName, name, userImage, subject, title, topic, duration, style, voice }: CompanionComponentProps) => {
    ...
    return (
        <section className='flex flex-col h-[70vh]'>
            <section className='flex gap-8 flex-col md:flex-row'>
                ...
                <div className="user-section max-sm:flex-col">
                    <div className="user-avatar">
                        <Image src={userImage} alt={userName} width={150} height={150} className='rounded-lg' />
                        <p className="font-bold text-2xl">{userName}</p>
                    </div>
                    <Button className='btn-mic flex-row flex h-fit' disabled={callStatus !== CallStatus.ACTIVE} variant={'outline'} onClick={toggleMic}>
                        <Image src={isMuted ? '/icons/mic-off.svg' : '/icons/mic-on.svg'} alt='mic' width={36} height={36} />
                        <p className="max-sm:hidden text-2xl font-bold">{isMuted ? 'Unmute' : 'Mute'}</p>
                    </Button>
                    <Button className={cn('rounded-lg py-2 cursor-pointer transition-colors w-full text-white',
                        callStatus === CallStatus.ACTIVE ? 'bg-red-500' : 'bg-primary', callStatus === CallStatus.CONNECTING && 'animate-pulse')} onClick={callStatus === CallStatus.ACTIVE ? handleDisconnect : handleCall}>
